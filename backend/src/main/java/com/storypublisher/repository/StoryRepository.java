...
public interface StoryRepository extends JpaRepository<Story, Long> {
    ...
    // Override findById to always fetch author and category
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.id = :id")
    Optional<Story> findByIdWithAuthor(@Param("id") Long id);
    
    // Find stories by author (user-specific filtering)
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.author = :author")
    Page<Story> findByAuthor(@Param("author") User author, Pageable pageable);
    
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.author = :author AND s.status = :status")
    ...
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.status = :status AND s.privacy = :privacy")
    Page<Story> findByStatusAndPrivacy(@Param("status") StoryStatus status, @Param("privacy") StoryPrivacy privacy, Pageable pageable);
    
    // Check if story with title already exists for author
    boolean existsByTitleAndAuthor(String title, User author);
    
    // Find story by ID and author (ownership validation)
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.id = :id AND s.author = :author")
    Optional<Story> findByIdAndAuthor(@Param("id") Long id, @Param("author") User author);
    
    // Find public story by slug (for public viewing)
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.slug = :slug AND s.status = :status AND s.privacy = :privacy")
    ...
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.slug = :slug")
    Optional<Story> findBySlug(@Param("slug") String slug);
    
    // Search stories by title or content
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.status = :status AND s.privacy = :privacy AND " +
    ...
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.category.id = :categoryId AND s.status = :status AND s.privacy = :privacy")
    ...
    @Query("SELECT s FROM Story s JOIN FETCH s.author LEFT JOIN FETCH s.category WHERE s.status = :status AND s.privacy = :privacy ORDER BY s.likeCount DESC")
